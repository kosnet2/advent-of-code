...
int main() {
	...
	for (auto c : wire1vec) {
		...
		while (steps) {
			...
			if (!mmap[{x, y}])
				mmap[{x, y}] = total_steps;
			steps--;
		}
	}

	for (auto c : x = y = total_steps = 0, wire2vec) {
		int steps = c.steps;
		while (steps) {
			total_steps++;
			if      (c.dir == "U")  y++;
			...
			if (mmap[{ x, y }]) {
				int total = mmap[{x, y}] + total_steps;
				if (total< minimum)
					minimum = total;
			}
			steps--;
